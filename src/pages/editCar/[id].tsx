...
import { UserCarsById } from "@src/api/api";
...
export default function EditCargo() {
  ...
  const { query } = router;
  const id = query?.id; // Check if query exists before accessing its properties
  const { token } = useAuth();
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await UserCarsById(token, idL);
